<?php
/**
 * Страница вывода подкатегории чертежа
 *
 * @author     <pa-nic@yandex.ru>
 * @version    1.0
 * @package    prints
 */

class printsBlueprintsSubcategoryAction extends noxThemeAction
{
    public $category;
    public $subCategory;

    public $blueprintModel;

    public $vectorModel;
    public $setModel;

    public function __construct(){
        $this->vectorModel = new printsVectorModel();
        $this->setModel = new printsSetModel();
        parent::__construct();
    }

    public function execute()
    {
        $categoryModel = new printsCategoryModel();
        $categoryUrl = $this->getParam('category', '');
        $subCategoryUrl = $this->getParam('subcategory', '');
        $this->category = $categoryModel->getByUrl($categoryUrl);

        if(!$this->category) {
            return 404;
        }

        $isCars = $this->category['db_table'] == 'car';
        $blueprintModel = new printsBlueprintModel();
        $categoryClassTable = 'prints_class_' . $this->category['db_table'];
        $dataCategoryModel = new noxModel(false, $categoryClassTable);

        $subCategoryName = false;

        $rastr = [];
        $vectors = [];
        $requestVectorModel = new printsRequestVectorModel();
        $config = noxConfig::getConfig();

        if(isset($dataCategoryModel->fields['make_id'])) {
            $subCategoryName = 'make_id';
            $makeModel = new printsMakeModel();
            $this->subCategory = $makeModel->getByUrl($subCategoryUrl, $this->category['id']);

            if(!$this->subCategory) {
                return 404;
            }

            $url = $config['protocol'] . $config['host'] . '/car-blueprints/' . $this->subCategory['url'];
            $this->addVar('url', $url);

            $subCategories = $makeModel->getActiveByCategory($this->category['id']);
            $this->addVar('subCategories', $subCategories);

            $res = $dataCategoryModel->where(['make_id' => $this->subCategory['id']])->fetchAll('id');
            $ids = array_keys($res);
            if($res) {
                $rastr = $blueprintModel->select('id, url, sort_name, full_name, views')->where(['item_id' => $ids, 'class_id' => $this->category['id']])->fetchAll();
                $vectors = (new printsVectorModel())->select('id, url, sort_name, full_name, views, prepay')->where(['item_id' => $ids, 'class_id' => $this->category['id']])->fetchAll();
            }

            if($isCars) { // cars
                $this->templateFileName = $this->moduleFolder . '/templates/frontend/blueprintsCarsSubcategory.html';
                $page = $_GET['page'];
                $brv = new noxModel(null, 'brv_summary_car');
                $brv->where(['make_id' => $this->subCategory['id']]);
                $this->addVar('pager', (new kafPager('pager2.html'))->create2($count = $brv->count(), $onPage = 100, 9, $page));
                $this->addVar('count', $count);
                if($page > 1){
                    $this->addVar('prevPage', $url . '?page=' . ($page - 1));
                }
                if($page * $onPage < $count){
                    $this->addVar('nextPage', $url . '?page=' . ($page + 1));
                }
                $bp = $brv->order('`sort_name`, `type`')->limit(($page - 1) * $onPage, $onPage)->fetchAll();
            } else {
                $requestVectorModel->where(['make_id' => $this->subCategory['id'], 'vector_id' => NULL]);
                $requestedVectors = $requestVectorModel->select('id, url, sort_name, full_name, "15" as views')->fetchAll();
            }

            $this->addVar('subCategory', $this->subCategory);
            $this->title = $this->caption = $this->subCategory['name'] . ' blueprints';
        }
        else {
            $alphabet = getLatinAlphabet($this->category['id'], $blueprintModel);
            $abc = strtoupper($subCategoryUrl);
            if(!in_array($abc, $alphabet)) {
                return 404;
            }
            $this->addVar('subCategories', $alphabet);
            $rastr = $blueprintModel->where('`class_id` = "' . $this->category['id'] . '" AND name LIKE "' . $subCategoryUrl . '%"')->fetchAll('item_id');
            $vectors = $this->vectorModel->where('`class_id` = "' . $this->category['id'] . '" AND name LIKE "' . $subCategoryUrl . '%"')->fetchAll('item_id');
            $requestVectorModel->where('`category_id` = "' . $this->category['id'] . '" AND name LIKE "' . $subCategoryUrl . '%" AND vector_id IS NULL');
            $requestedVectors = $requestVectorModel->fetchAll();
        }

        if(isset($requestedVectors)) {
            $blueprints = [];
            foreach($rastr as $ar) {
                $ar['type'] = Prints::BLUEPRINT;
                $ar['bold'] = false;
                $ar['tag'] = '';
                $ar['seo'] = 'blueprints free';
                $blueprints[] = $ar;
            }
            foreach($vectors as $ar) {
                $ar['type'] = $ar['prepay'] ? Prints::PREPAY : Prints::VECTOR;
                $ar['bold'] = true;
                $ar['tag'] = $ar['prepay'] ? Prints::HTML_ICON_PREPAY : Prints::HTML_ICON_VECTOR;
                $ar['seo'] = $ar['prepay'] ? 'blueprints' : 'blueprints and drawings';
                $blueprints[] = $ar;
            }
            foreach($requestedVectors as $ar) {
                $ar['type'] = Prints::REQUEST_VECTOR;
                $ar['bold'] = true;
                $ar['tag'] = Prints::HTML_ICON_VECTOR_REQUEST;
                $ar['seo'] = 'blueprints request';
                $blueprints[] = $ar;
            }

            function sortBp($a1, $a2) {
                $resp = strcasecmp($a1['sort_name'], $a2['sort_name']);
                if(!$resp) {
                    static $typeRange;
                    if(!$typeRange) $typeRange = [Prints::VECTOR => 1, Prints::PREPAY => 2, Prints::REQUEST_VECTOR => 3, Prints::BLUEPRINT => 4];
                    $resp = $typeRange[$a1['type']] - $typeRange[$a2['type']];
                }
                return $resp;
            }

            usort($blueprints, 'sortBp');
            $this->addVar('blueprints', $blueprints);
        } elseif (isset($bp)){
            foreach ($bp as &$ar){
                switch ($ar['type']){
                    case 0:
                        $ar['type'] = Prints::VECTOR;
                        $ar['bold'] = true;
                        $ar['tag'] = Prints::HTML_ICON_VECTOR;
                        $ar['seo'] = 'blueprints and drawings';
                        break;
                    case 1:
                        $ar['type'] = Prints::PREPAY;
                        $ar['bold'] = true;
                        $ar['tag'] = Prints::HTML_ICON_PREPAY;
                        $ar['seo'] = 'blueprints';
                        break;
                    case 2:
                        $ar['type'] = Prints::REQUEST_VECTOR;
                        $ar['bold'] = true;
                        $ar['tag'] = Prints::HTML_ICON_VECTOR_REQUEST;
                        $ar['seo'] = 'blueprints request';
                        break;
                    case 3:
                        $ar['type'] = Prints::BLUEPRINT;
                        $ar['bold'] = false;
                        $ar['tag'] = '';
                        $ar['seo'] = 'blueprints free';
                        break;
                }
            }
            $this->addVar('blueprints', $bp);
        }

        if($_GET['search'] != '') {
            $raw = $_GET['search'];
            $params = [
                'category_id' => $this->category['id'],
            ];

            if($subCategoryName) {
                $params[$subCategoryName] = $this->subCategory['id'];
            }

            if(@$raw['name']) {
                $params['name'] = $raw['name'];
            }
            if(@$raw['year']) {
                $params['year'] = $raw['year'];
            }

            $page = $_GET['page'];
            $onPage = 100;
            $search = (new printsSearchModel())->searchSphinx2(false, $params, ($page - 1) * $onPage, $onPage);
            $this->addVar('pager', (new kafPager('pager2.html'))->create2($search['total'], $onPage, 9, $page));
            $this->addVar('search', $search);
            if(isset($url)){
                $this->addVar('canonical', $url);
            }
        }

        if(isset($dataCategoryModel->fields['make_id'])) {
            $make = $this->subCategory['name'];
            $this->addVar('sectionMainName', $make);

            //====================TAGS=============================
            $useTagsModel = new tagCategoryUseModel();
            $keywords = '';
            foreach(explode(',', tagBlueprintModel::$rasterTags) as $tag) {
                $keywords .= $this->subCategory['name'] . ' ' . trim($tag) . ', ';
            }
            $useTags = $useTagsModel->getById($this->category['id']);
            foreach($useTags as $tag) {
                $keywords .= $this->subCategory['name'] . ' ' . $tag . ', ';
            }
            $keywords .= implode(', ', (new tagMakeModel)->getById($this->subCategory['id'])) . ', ';
            $keywords .= tagCategoryUseModel::$useTags . ', ' . tagBlueprintModel::$rasterAdsTags;
            $this->addMetaKeywords($keywords);
            //====================/TAGS=============================
            $this->vectorModel->reset();
            $src = array_column($vectors, 'id');
            if($isCars){
                $state = [
                    'titleVector' => false,
                    'relVectors' => [],
                    'sets' => []
                ];
                //noxSystem::$console->log(json_encode($src));
                if($src){
                    $paymentModel = new paymentModel();
                    $paidVectors = $paymentModel->select('DISTINCT `purchase_id`')
                        ->where(['status' => 'approved', 'purchase_id' => $src])
                        ->fetchAll(null, 'purchase_id');
                    if($paidVectors){
                        $src = array_values(array_diff($src, $paidVectors));
                        $state = $this->execVectors($state, $paidVectors);
                    }
                }
                //noxSystem::$console->log(json_encode($src));
                if($src && (count($state['relVectors']) < 4)){
                    $readyVectors = $this->vectorModel->reset()
                        ->where(['prepay' => '0', 'id' => $src])->fetchAll(null, 'id');
                    if($readyVectors){
                        $src = array_values(array_diff($src, $readyVectors));
                        $state = $this->execVectors($state, $readyVectors);
                    }
                }
                //noxSystem::$console->log(json_encode($src));
                if($src && (count($state['relVectors']) < 4)){
                    $prepayVectors = $this->vectorModel->reset()
                        ->where(['prepay' => '1', 'id' => $src])->fetchAll(null, 'id');
                    if($prepayVectors){
                        $state = $this->execVectors($state, $prepayVectors);
                    }
                }
                if($state['titleVector']){
                    if(isset($state['titleVector']['preview'])){ // check on prepay
                        $img_url = noxSystem::$media -> src($state['titleVector']['preview']);
                        $this->addVar('img_url', $img_url);
                        $this->addVar('image_src', $img_url);
                        $state['titleVector']['preview'] = $img_url;
                    } else {
                        $state['titleVector']['preview'] = '';
                    }
                    $this->addVar('titleVector', $state['titleVector']);
                }
                foreach ($state['relVectors'] as &$row) {
                    $row['preview'] = isset($row['preview']) ? noxSystem::$media->srcMini($row['preview']) : '';
                }
                $this->addVar('relVectors', $state['relVectors']);
            } else {
                $relVectorCount = 1 + max(0, floor((count($blueprints) - 12) / 10.2));
                $relVectors = $this->vectorModel->getRelated($relVectorCount, ['id' => $src], ['class_id' => $this->category['id']]);
                $this->addVar('relVectors', $relVectors);
            }
        }
        else {
            $this->addVar('sectionMainName', $this->category['name']);
            //====================TAGS=============================
            $make = $this->category['name_singular'];
            $useTagsModel = new tagCategoryUseModel();
            $keywords = '';
            foreach(explode(',', tagBlueprintModel::$rasterTags) as $tag) {
                $keywords .= $this->category['name_singular'] . ' ' . trim($tag) . ', ';
            }
            $useTags = $useTagsModel->getById($this->category['id']);
            foreach($useTags as $tag) {
                $keywords .= $this->category['name_singular'] . ' ' . $tag . ', ';
            }
            $keywords .= implode(', ', (new tagCategoryModel)->getById($this->category['id'])) . ', ';
            $keywords .= tagCategoryUseModel::$useTags . ', ' . tagBlueprintModel::$rasterAdsTags;
            $this->addMetaKeywords($keywords);
            //====================/TAGS=============================
            $relVectorCount = 1 + max(0, floor((count($blueprints) - 12) / 10.2));
            $this->addVar('relVectors', $this->vectorModel->getRelated($relVectorCount, ['class_id' => $this->category['id']]));
        }
        if(isset($page) && $page > 1 && isset($count) && isset($onPage)){
            $this->title = $make . ' blueprints: download all models for 3D modeling or wrap - Page  ' . $page;
            $metaDescription = '';
        } else {
            $class = ($isCars || isset($abc)) ? '' : (' ' . strtolower($this->category['name']));
            $postFix = isset($abc) ? (' - ' . $abc) : '';
            $this->title = $make . $class . ' blueprints collection, download all models for 3D modeling or wrap' . $postFix;
            if($isCars && isset($count)){
                $metaDescription = 'Collection of ' . $count . ' ' . $this->subCategory['name'] . ' blueprints highly '
                    . 'detailed for 3D design, modeling and visualization. Download free blueprints by carmaker '
                    . $this->subCategory['name'] . '. Get vector templates for' . $this->subCategory['name'] .
                    ' wrap, vehicle branding and corporate design wrapping. Use it for 3D scene, T-shirt, poster or '
                    . 'whatever. Choose between different car blueprints available or Request any other '
                    . $this->subCategory['name'] . ' blueprint.';
            } else {
                $metaDescription = 'Highly detailed ' . $make . ' vector blueprint collection for 3D scene '
                    . 'and visualizations. Editable templates for ' . $make . ' wrap, vehicle branding and corporate '
                    . 'design wrapping. Use it for T-shirt, birthday cake, poster or whatever. Choose between different '
                    . 'blueprints available or Request any other ' . $make . ' blueprint.';
            }
        }
        if($isCars){
            $desc = 'Highly detailed ' . $make . ' vector blueprint collection '
                . 'for 3D design, modeling and visualization. Download free blueprints by carmaker '
                . $make . '. Get vector templates for ' . $make .
                ' wrap, vehicle branding and corporate design wrapping. Use it for 3D scene, T-shirt, poster or '
                . 'whatever. Choose between different car blueprints available or Request any other '
                . $make . ' blueprint.';
            $this->addVar('desc', $desc);
        }
        $this->addVar('metaDescription', $metaDescription);
        $this->addMetaDescription($metaDescription);
        $this->caption = $make . (isset($alphabet) ? (' '. $abc) : '') . ' blueprints';

        setcookie('my_search_q', '', time() - 1000, '/');
        setcookie('my_search_model',
            $this->category['id'] . '::'
            . (isset($this->subCategory['id']) ? $this->subCategory['id'] : '') . '::'
            . (isset($params['name']) ? $params['name'] : '') . '::::'
            . (isset($params['year']) ? $params['year'] : '')
            , 0, '/'
        );

        $this->addVar('subCategoryName', $subCategoryName);
        $breadcrumbs = [
            [
                'name' => 'blueprints',
                'title' => 'blueprints',
                'url'   => '/blueprints'
            ],
            [
                'name' => strtolower($this->category['name_singular']) . ' blueprints',
                'title' => strtolower($this->category['name_singular']) . ' blueprints',
                'url'   => '/' . $this->category['url'] . '-blueprints/'
            ],
        ];


        $this->addVar('breadcrumbs', $breadcrumbs);
        $this->addVar('category', $this->category);
        $this->addVar('subCategory', $this->subCategory);
    }

    public function execVectors($state, $vectors){
        shuffle($vectors);
        if(!$state['titleVector']){
            $state['titleVector'] = $this->tryGetSet(array_splice($vectors, 0, 1)[0]);
            if($state['titleVector']['set']){
                $state['sets'][] = $state['titleVector']['id'];
            }
        }
        while((count($state['relVectors']) < 4) && count($vectors)){
            if($res = $this->tryGetSet(array_splice($vectors, 0, 1)[0], $state['sets'])){
                if($res['set']){
                    $state['sets'][] = $res['id'];
                }
                $state['relVectors'][] = $res;
            }
        }
        return $state;
    }

    public function tryGetSet($vectorId, $ignore = []){
        if($res = $this->setModel->getSetForVector($vectorId)){
            if(!in_array($res['id'], $ignore)){
                $res['c'] = count($vectorsInSet = $this->setModel->setVectorModel->getVectorsIdBySet($res['id']));
                $res['price'] = $this->vectorModel->where(['id' => $vectorsInSet])->order('price')->limit(1)->fetch('price');
                $vector = $this->vectorModel->where(['id' => $vectorId])->fetch();
                $res['preview'] = $vector['preview'];
                $res['vector'] = !$vector['prepay'];
                $res['set'] = true;
            } else {
                return false;
            }
        } else {
            $res = $this->vectorModel->getById($vectorId);
            $res['set'] = false;
        }
        return $res;
    }
}
